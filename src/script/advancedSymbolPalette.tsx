/**
 * Advanced Symbol Palette with Dynamic Palette Management
 * Features:
 * - Add custom palettes with shapes, Git links, and documentation
 * - Remove existing palettes at runtime
 * - Manage predefined and custom palettes
 * - Persistent palette configuration
 */
...
import { ListViewComponent, SelectedCollection } from '@syncfusion/ej2-react-lists';
...
// Helper function to construct asset URL from public folder
// Assets in public/ folder are served directly from the root path
const getShapeImagePath = (paletteId: string): string => {
    return `assets/dbstyle/shapes_images/${paletteId}.png`;
};
...
export const AdvancedSymbolPalette = () => {
    // Initialize shape providers
    const floorplanShapesProvider: FloorplanShapes = new FloorplanShapes();
    const networkShapesProvider: NetworkShapes = new NetworkShapes();

    // Component references
    const symbolPaletteRef = useRef<SymbolPaletteComponent>(null);
    const manageShapesDialogRef = useRef<DialogComponent>(null);
    const paletteListViewRef = useRef<ListViewComponent>(null);
    // ====== STATE MANAGEMENT ======

    /**
     * All available palettes (predefined + custom)
     * Flow palette is enabled by default
     */
    const [availablePalettes, setAvailablePalettes] = useState<PaletteMetadata[]>([
        {
            id: 'Flow',
            title: 'Flow Shapes',
            displayName: 'Flow Shapes',
            iconCss: 'e-icons e-flow',
            enabled: true,
            isActive: false,
            isCustom: false,
            shapes: createFlowShapes()
        },
        {
            id: 'Basic',
            title: 'Basic Shapes',
            ...
            isActive: false,
            isCustom: false,
            shapes: createBasicShapes()
        },
        {
            id: 'Bpmn',
            title: 'BPMN Shapes',
            displayName: 'BPMN Shapes',
            iconCss: 'e-icons e-flow',
            enabled: true,
            isActive: false,
            isCustom: false,
            shapes: createBPMNShapes()
        },
        {
            id: 'Connectors',
            title: 'Connectors',
            displayName: 'Connector Shapes',
            iconCss: 'e-icons e-line',
            enabled: true,
            isActive: false,
            isCustom: false,
            shapes: createConnectors()
        },

        {
            id: 'Network',
            title: 'Network Shapes',
            displayName: 'Network Diagram Shapes',
            iconCss: 'e-icons e-network',
            enabled: false,
            isActive: false,
            isCustom: false,
            shapes: networkShapesProvider.getNetworkShapes()
        },
        {
            id: 'Floorplan',
            title: 'Floorplan Shapes',
            displayName: 'Floorplan Shapes',
            iconCss: 'e-icons e-home',
            enabled: false,
            isActive: false,
            isCustom: false,
            shapes: floorplanShapesProvider.getFloorPlans()
        },
        {
            id: 'Electrical',
            title: 'Electrical Shapes',
            displayName: 'Electrical Shapes',
            iconCss: 'e-icons e-component',
            enabled: false,
            isActive: false,
            isCustom: false,
            shapes: createElectricalShapes()
        }
    ]);

    // Manage dialog visibility state
    const [isManageShapesDialogOpen, setIsManageShapesDialogOpen] = useState(false);

    // Preview image state - defaults to 'flow' palette
    const [selectedPreviewPaletteId, setSelectedPreviewPaletteId] = useState('Flow');

    /**
     * Apply selected palettes from the manage dialog ListView to the symbol palette
     */
    const applyManageDialogSelection = () => {
        if (!paletteListViewRef.current || !symbolPaletteRef.current) {
            setIsManageShapesDialogOpen(false);
            return;
        }

        const selectedItems: SelectedCollection = paletteListViewRef.current.getSelectedItems() as SelectedCollection;
        const selectedPaletteIds: string[] = selectedItems.text as string[] || [];

        // Update palette states based on selected ids
        setAvailablePalettes((previousPalettes) =>
            previousPalettes.map((paletteItem) => ({
                ...paletteItem,
                enabled: selectedPaletteIds.indexOf(paletteItem.id) !== -1
            }))
        );

        // Also directly update the SymbolPaletteComponent to reflect selection
        const enabledPalettesToDisplay = availablePalettes
            .filter((paletteItem) => selectedPaletteIds.indexOf(paletteItem.id) !== -1)
            .map((paletteItem) => ({
                id: paletteItem.id,
                expanded: true,
                symbols: paletteItem.shapes,
                title: paletteItem.title,
            }));

        if (symbolPaletteRef.current) {
            (symbolPaletteRef.current as any).palettes = enabledPalettesToDisplay;
            (symbolPaletteRef.current as any).dataBind && (symbolPaletteRef.current as any).dataBind();
        }
        setIsManageShapesDialogOpen(false);
    };

    function createFlowShapes(): NodeModel[] {
        return [
    ...
    function createBasicShapes(): NodeModel[] {
        return [
    ...
    function createBPMNShapes(): NodeModel[] {
    ...
    function createElectricalShapes(): NodeModel[] {
    ...
    /**
     * Generate currently enabled palettes for display in the symbol palette
     * Filters to show only enabled palettes with their shapes
     */
    const getDisplayedPalettes = () => {
        return availablePalettes
            .filter((paletteItem) => paletteItem.enabled)
            .map((paletteItem) => ({
                id: paletteItem.id,
                expanded: true,
                symbols: paletteItem.shapes,
    ...
    /**
     * Maintain debounced display palettes state to avoid Syncfusion timing issues
     * during rapid palette updates
     */
    const [displayedPalettes, setDisplayedPalettes] = useState(getDisplayedPalettes());

    useEffect(() => {
        const debounceTimer = setTimeout(() => {
            setDisplayedPalettes(getDisplayedPalettes());
        }, 50);
        return () => clearTimeout(debounceTimer);
    }, [availablePalettes]);

    // ====== EVENT HANDLERS - MANAGE DIALOG ======

    /**
     * Handle manage palettes button click - opens the manage dialog
     */
    const handleManagePalettesClick = (mouseEvent: React.MouseEvent) => {
        mouseEvent.preventDefault();
        setIsManageShapesDialogOpen(true);
    };

    /**
     * Handle manage dialog close - triggered when user closes dialog
     */
    ...
    return (
        <div className="symbol-palette-inner">
            {/* Symbol Palette */}
            <SymbolPaletteComponent
                id="symbolpalette"
            ...
            {/* More Shapes Button */}
            <div className="more-shapes-btn" onClick={handleManagePalettesClick}>
                <div className="more-shapes-content">
                    <span className="e-icons e-add-icon more-shapes-icon"></span>
                    More Shapes
                </div>
            </div>

            {/* ============ SHAPES DIALOG (MORE SHAPES) ============ */}
            <DialogComponent
                id="moreShapesDialog"
                ref={manageShapesDialogRef}
                header="Shapes"
                visible={isManageShapesDialogOpen}
                showCloseIcon={true}
                width="700px"
                height="500px"
                isModal={true}
                ...
                buttons={[
                    ...
                    },
                    {
                        ...
                        click: () => applyManageDialogSelection()
                    }
                ]}
            >
                <div className="shapes-dialog-inner">
                    <div className="shapes-dialog-left">
                        <ListViewComponent
                            id="moreShapesList"
                            ref={paletteListViewRef}
                            dataSource={availablePalettes.map((paletteItem) => ({ text: paletteItem.id, id: paletteItem.id, checked: paletteItem.enabled }))}
                            fields={{ text: 'text', isChecked: 'checked' }}
                            showCheckBox={true}
                            select={(selectArgs) => {
                                const selectedPaletteId = (selectArgs as any).text as string;
                                if (selectedPaletteId) setSelectedPreviewPaletteId(selectedPaletteId);
                            }}
                        />
                    </div>

                    {/* Right Panel - Shape Preview Image */}
                    <div className="shapes-dialog-right">
                        {/* Display preview image for selected palette */}
                        {selectedPreviewPaletteId ? (
                            <img
                                key={selectedPreviewPaletteId}
                                id="shapePreviewImage"
                                src={getShapeImagePath(selectedPreviewPaletteId)}
                                alt={`${selectedPreviewPaletteId} shapes preview`}
                                className="shape-preview-image"
                            />
                        ) : (
                            <div className="shape-preview-placeholder">
                                <p>Select a shape category to preview</p>
                            </div>
                        )}
                    </div>
                </div>
